...
try_source(){ R="$1"; S="$2"; NAME="$3"; get "$R" "$TMPJ" || return 1; get "$S" "$TMPS" || return 1; OUT="$(verify_pair online "$TMPJ" "$TMPS" 2>&1)"; RC=$?; [ $RC -eq 0 ] || { printf '%s\n' "$OUT"; return $RC; }; mv -f "$TMPJ" "$CACHE_JSON" || return 1; mv -f "$TMPS" "$CACHE_SIG" || return 1; EPOCH="$(date +%s 2>/dev/null || echo 0)"; { echo "SYNC_EPOCH=$EPOCH"; echo "SYNC_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || true)"; echo "SOURCE=$NAME"; } > "$SYNC_STATE.tmp" && mv -f "$SYNC_STATE.tmp" "$SYNC_STATE"; printf '%s;SOURCE=%s\n' "$OUT" "$NAME"; return 0; }
