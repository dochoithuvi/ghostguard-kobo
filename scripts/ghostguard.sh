#!/bin/sh
# DCPRO GhostGuard Kobo v0.8.7.1 Safety Lockdown.
# Auto Learn -> approval -> probation -> Shadow. Protect is deliberately disabled.
set -u
...
stop_engine(){
  ...
  [ -f "$PIDFILE" ] && { P="$(cat "$PIDFILE" 2>/dev/null)"; [ -n "$P" ] && kill -TERM "$P" 2>/dev/null || true; }
  sleep 1; rm -f "$PIDFILE" "$CHILDPID" "$ARMFILE" "$FILTERFILE" "$WATCHFILE"
  [ -x "$PROFILE_MGR" ] && "$PROFILE_MGR" session-end >/dev/null 2>&1 || true
  profile_sync; write_safe_state
  echo "GhostGuard đã dừng; Safety Lockdown vẫn giữ Protect OFF."
}

start_engine(){
  REQUEST="$1"; write_device_info; write_safe_state
  ...
  case "$REQUEST" in
    LEARN|AUTO)
      case "$PSTATE" in CALIBRATION|'') MODE=LEARN;; *) MODE=SHADOW;; esac
      ;;
    SHADOW) MODE=SHADOW;;
    PROTECT) MODE=SHADOW; echo "Protect bị khóa ở v0.8.7.1; chuyển sang SHADOW.";;
    *) MODE=SHADOW;;
  esac
  if is_running; then
    CUR="$(cat "$MODEFILE" 2>/dev/null)"
    [ "$CUR" = "$MODE" ] && { echo "GhostGuard đang chạy: $MODE"; return 0; }
    stop_engine >/dev/null 2>&1; sleep 1
  fi
  BIN="$(binary_path)"; [ -n "$BIN" ] && [ -x "$BIN" ] || { echo "Không có native binary phù hợp."; return 5; }
  rm -f "$ARMFILE" "$FILTERFILE" "$WATCHFILE" "$REBINDFILE" 2>/dev/null || true
  ...
  if is_running; then [ -x "$PROFILE_MGR" ] && "$PROFILE_MGR" session-start >/dev/null 2>&1 || true; echo "GhostGuard đã chạy: $MODE (Safety Lockdown; Protect OFF)"; return 0; fi
...
approve_profile(){ write_device_info; license_check || { echo "License chưa hợp lệ."; return 3; }; [ -x "$PROFILE_MGR" ] || return 5; "$PROFILE_MGR" approve || return $?; echo SHADOW > "$MODEFILE"; if is_running && [ -f "$CHILDPID" ]; then C="$(cat "$CHILDPID" 2>/dev/null)"; [ -n "$C" ] && kill -TERM "$C" 2>/dev/null || true; fi; write_safe_state; echo "Profile đã kích hoạt. Protect vẫn bị khóa bởi Safety Lockdown."; }
