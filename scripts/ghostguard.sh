#!/bin/sh
# DCPRO GhostGuard Kobo v0.8.7 Safety Handshake.
# Auto Learn -> approval -> probation -> guarded Protect.
set -u
...
MODEFILE="$RUN/mode"; INPUTFILE="$RUN/input_device"; ARMFILE="$RUN/PROTECT_ARMED"; FILTERFILE="$RUN/PROTECT_FILTER_ARMED"; WATCHFILE="$RUN/PROTECT_WATCHDOG"; REBINDFILE="$RUN/nickel_rebind_once"
...
stop_engine(){
  ...
  sleep 1; rm -f "$PIDFILE" "$CHILDPID" "$ARMFILE" "$FILTERFILE" "$WATCHFILE"
  [ -x "$PROFILE_MGR" ] && "$PROFILE_MGR" session-end >/dev/null 2>&1 || true
  profile_sync
  echo "GhostGuard đã dừng; Protect được nhả fail-open."
}
...
start_engine(){
  ...
  if is_running; then
    ...
    if [ "$CUR" = "$MODE" ]; then
      case "$MODE" in
        PROTECT)
          if [ -r "$PROTECT_STATUS" ] && grep -Eq '^STATE=(NICKEL_VIRTUAL_NOT_OPEN|VIRTUAL_EVENT_NOT_FOUND|SAFETY_FAIL_OPEN)$' "$PROTECT_STATUS" 2>/dev/null; then
  ...
  rm -f "$ARMFILE" "$FILTERFILE" "$WATCHFILE" "$REBINDFILE"; [ "$MODE" != PROTECT ] && rm -f "$PROTECT_STATUS" 2>/dev/null || true
  ...
  if is_running; then
    [ -x "$PROFILE_MGR" ] && "$PROFILE_MGR" session-start >/dev/null 2>&1 || true
    echo "GhostGuard đã chạy: $MODE"
    [ "$MODE" = PROTECT ] && echo "Safety Handshake: virtual touch -> Nickel fd -> post-grab forward -> filter arm. Chạm màn hình một lần để hoàn tất preflight."
    return 0
  fi
...
approve_profile(){ write_device_info; license_check || { echo "License chưa hợp lệ."; return 3; }; [ -x "$PROFILE_MGR" ] || return 5; "$PROFILE_MGR" approve || return $?; echo SHADOW > "$MODEFILE"; if is_running && [ -f "$CHILDPID" ]; then C="$(cat "$CHILDPID" 2>/dev/null)"; [ -n "$C" ] && kill "$C" 2>/dev/null || true; fi; echo "Profile đã kích hoạt. Probation 0/2; Protect chỉ bật sau Probation và Safety Handshake."; }
status_full(){ "$BASE/nm_quick.sh" status; echo; [ -r "$PROTECT_STATUS" ] && { echo '--- Protect ---'; cat "$PROTECT_STATUS"; }; [ -r "$DATA/HANDSHAKE_STATUS.ggstate" ] && { echo '--- Handshake ---'; cat "$DATA/HANDSHAKE_STATUS.ggstate"; }; [ -r "$DATA/PROXY_STATUS.ggstate" ] && { echo '--- Proxy ---'; cat "$DATA/PROXY_STATUS.ggstate"; }; [ -r "$PROFILE_V5" ] && { echo '--- Profile V5 ---'; cat "$PROFILE_V5"; }; [ -r "$LICENSE_STATUS" ] && { echo '--- License ---'; cat "$LICENSE_STATUS"; }; }
