...
# Return 20 when a freshly downloaded, correctly signed registry explicitly
# denies this device/entitlement. That must never fall back to an older cache.
fresh_deny(){
    case "$1" in
...
try_source(){
    ...
    if [ $RC -ne 0 ]; then
        printf '%s\n' "$OUT"
        fresh_deny "$OUT" && return 20
        return $RC
    fi
    mv -f "$TMPJ" "$CACHE_JSON" || return 1
    mv -f "$TMPS" "$CACHE_SIG" || return 1
    EPOCH="$(date +%s 2>/dev/null || echo 0)"
    { echo "SYNC_EPOCH=$EPOCH"; echo "SYNC_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || true)"; echo "SOURCE=$NAME"; } > "$SYNC_STATE.tmp" && mv -f "$SYNC_STATE.tmp" "$SYNC_STATE"
    printf '%s;SOURCE=%s\n' "$OUT" "$NAME"
    return 0
}

try_source "$REG_PRIMARY" "$SIG_PRIMARY" GITHUB_RAW; RC=$?
[ $RC -eq 0 ] && exit 0
[ $RC -eq 20 ] && exit 1
rm -f "$TMPJ" "$TMPS" 2>/dev/null
try_source "$REG_MIRROR" "$SIG_MIRROR" JSDELIVR; RC=$?
[ $RC -eq 0 ] && exit 0
[ $RC -eq 20 ] && exit 1
rm -f "$TMPJ" "$TMPS" 2>/dev/null
