#!/bin/sh
# DCPRO GhostGuard Kobo v0.8.5 Ghost Capture fast Status. Network update check is background-only.
set -u
...
show_status(){
  ...
  echo 'GhostGuard Kobo 0.8.5 Ghost Capture';echo "Engine: $ENG | Auto mode: $MODE";echo "License: $(license_summary)";update_summary;echo "Profile: $(friendly "$PS")"
  ...
  echo 'Fail-open: ON'
  case "$PS" in CALIBRATION|'')echo 'Next: tiếp tục dùng máy bình thường.';;PENDING_APPROVAL)echo 'Next: GhostGuard - Start để tự kích hoạt Profile.';;PROBATION)echo 'Next: Start/Stop đủ 2 phiên Probation.';;PROBATION_PASSED) case "$PSTAT" in ACTIVE)echo 'Next: GhostGuard đang bảo vệ và ghi Ghost Capture.';;NICKEL_REBINDING|VIRTUAL_READY_WAITING_FOR_NICKEL)echo 'Next: chờ Nickel khởi động lại và mở Status.';;NICKEL_VIRTUAL_NOT_OPEN|VIRTUAL_EVENT_NOT_FOUND)echo 'Next: GhostGuard - Start để thử rebind lại.';;*)echo 'Next: GhostGuard - Start để bật Protect.';;esac;;esac
}
