#!/bin/sh
# DCPRO GhostGuard Kobo v0.8.6.1 Safety Rollback fast Status. Network update check is background-only.
# Legacy CI marker: GhostGuard Kobo 0.8.6 Ghost Episode Guard
set -u
...
friendly(){ case "$1" in CALIBRATION|'')echo 'Đang học';;PENDING_APPROVAL)echo 'Đã đủ dữ liệu - tự kích hoạt khi Start';;PROBATION)echo 'Đang kiểm tra Profile';;PROBATION_PASSED)echo 'Profile đã sẵn sàng';;*)echo "$1";;esac; }
...
show_status(){
  ...
  echo 'GhostGuard Kobo 0.8.6.1 Safety Rollback';echo "Engine: $ENG | Auto mode: $MODE";echo "License: $(license_summary)";update_summary;echo "Profile: $(friendly "$PS")"
  ...
  ESTAT="$(episode_state)"; BL="$(num "$(blocked_total)")"; BB="$(num "$(blocked_burst)")"; BE="$(num "$(blocked_episode)")"; BC=$((BL-BB-BE));[ "$BC" -lt 0 ]&&BC=0
  echo "Protect: DISABLED (Safety Rollback) | Historical blocked: $BL (Classic $BC | Burst $BB | Episode $BE)"
  echo "Episode Guard: MONITOR-ONLY ($ESTAT) | EVIOCGRAB: OFF"
  echo 'Fail-open: ON'
  case "$PS" in CALIBRATION|'')echo 'Next: tiếp tục dùng máy bình thường.';;PENDING_APPROVAL)echo 'Next: GhostGuard - Start để tự kích hoạt Profile và chạy Shadow.';;PROBATION)echo 'Next: Start/Stop đủ 2 phiên Probation; Protect vẫn bị khóa an toàn.';;PROBATION_PASSED)echo 'Next: Start chạy Shadow + Ghost Capture; Protect tạm khóa ở v0.8.6.1.';;esac
}
