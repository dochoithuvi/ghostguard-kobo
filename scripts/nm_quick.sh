#!/bin/sh
# DCPRO GhostGuard Kobo v0.8.7.1 Safety Lockdown fast Status.
set -u
...
show_status(){
  ...
  echo 'GhostGuard Kobo 0.8.7.1 Safety Lockdown';echo "Engine: $ENG | Auto mode: $MODE";echo "License: $(license_summary)";update_summary;echo "Profile: $(friendly "$PS")"
  ...
  echo "Protect: DISABLED (Safety Lockdown) | Historical blocked: $BL (Classic $BC | Burst $BB | Episode $BE)"
  echo "Episode Guard: MONITOR-ONLY ($ESTAT) | EVIOCGRAB: OFF"
  echo 'Start: SHADOW + Ghost Capture only | Fail-open: ON'
  case "$PS" in CALIBRATION|'')echo 'Next: tiếp tục dùng máy bình thường.';;PENDING_APPROVAL)echo 'Next: GhostGuard - Start để tự kích hoạt Profile và chạy Shadow.';;PROBATION)echo 'Next: Start/Stop đủ phiên Probation; Protect vẫn bị khóa.';;PROBATION_PASSED)echo 'Next: Start chạy Shadow + Ghost Capture; không arm Protect ở v0.8.7.1.';;esac
}
