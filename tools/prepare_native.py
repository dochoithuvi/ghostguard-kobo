...
new_process = '''static void protect_process(struct input_event*e){u32 nowsec=(u32)e->tv_sec,nowusec=(u32)e->tv_usec;if(e->type==EV_SYN&&e->code==SYN_DROPPED){qreset();suppress_tail=0;burst_hits=0;burst_guard=0;episode_hits=0;episode_multi_hits=0;episode_short_hits=0;episode_state(0);release_protect("STATE=SYN_DROPPED_FAIL_OPEN\\nPROTECT_ACTIVE=0\\nFAIL_OPEN=1\\n");process(e);return;}maybe_arm_on_frame(e);if(!protect_active){process(e);return;}if(suppress_tail){process(e);if(e->type==EV_SYN&&e->code==SYN_REPORT)suppress_tail=0;return;}if(!qactive&&contact_start_event(e)){burst_decay(nowsec,nowusec);episode_decay(nowsec,nowusec);qactive=1;qsec=nowsec;qusec=nowusec;qcount=0;qevents=0;qpassthrough=0;qmulti=0;}if(!qactive){process(e);forward_event(e);return;}if(qpassthrough){qevents++;process(e);if(raw_mt_count>1)qmulti=1;forward_event(e);if(last_end_happened){u32 elapsed=timeval_diff_us(qsec,qusec,last_end_sec,last_end_usec);u32 gap=episode_gap(last_end_sec,last_end_usec);burst_note(last_end_sec,last_end_usec,elapsed,last_end_risk);episode_note(last_end_sec,last_end_usec,elapsed,qmulti,gap);capture_contact(last_end_sec,last_end_usec,elapsed,last_end_risk,last_end_families,qevents,qmulti,gap,"ALLOW_PASSTHROUGH");qreset();}return;}if(qcount>=QMAX){qflush();qpassthrough=1;qevents++;process(e);if(raw_mt_count>1)qmulti=1;forward_event(e);return;}qbuf[qcount++]=*e;qevents++;process(e);if(raw_mt_count>1)qmulti=1;{u32 elapsed=timeval_diff_us(qsec,qusec,nowsec,nowusec);u32 hold=protect_hold_us();if(qmulti&&!episode_guard){qflush();qpassthrough=1;return;}if(last_end_happened){u32 gap=episode_gap(last_end_sec,last_end_usec);int classic_drop=(last_end_candidate&&elapsed<=hold);burst_note(last_end_sec,last_end_usec,elapsed,last_end_risk);episode_note(last_end_sec,last_end_usec,elapsed,qmulti,gap);if(classic_drop){capture_contact(last_end_sec,last_end_usec,elapsed,last_end_risk,last_end_families,qevents,qmulti,gap,"BLOCK_CLASSIC");log_block(last_end_sec,last_end_usec,last_end_risk,last_end_families);qreset();suppress_tail=1;return;}if(burst_should_drop(elapsed,last_end_risk,last_end_families)){capture_contact(last_end_sec,last_end_usec,elapsed,last_end_risk,last_end_families,qevents,qmulti,gap,"BLOCK_BURST");log_block_burst(last_end_sec,last_end_usec,last_end_risk,last_end_families);qreset();suppress_tail=1;return;}if(episode_should_drop(elapsed,last_end_risk,qmulti,gap)){capture_contact(last_end_sec,last_end_usec,elapsed,last_end_risk,last_end_families,qevents,qmulti,gap,"BLOCK_EPISODE");log_block_episode(last_end_sec,last_end_usec,last_end_risk,last_end_families);qreset();suppress_tail=1;return;}capture_contact(last_end_sec,last_end_usec,elapsed,last_end_risk,last_end_families,qevents,qmulti,gap,"ALLOW");qflush();qreset();return;}if(elapsed>=hold){qflush();qpassthrough=1;return;}}}'''

for old, new, label in (
    (old_release, new_release, "release"),
    (old_log, new_log, "block log"),
    (old_q, new_q, "adaptive queue + episode capture"),
    (old_qreset, new_qreset, "queue reset"),
    (old_process, new_process, "Ghost Episode Guard protect_process"),
):
    if old not in src:
