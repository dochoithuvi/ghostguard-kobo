...
new_process = '''static void protect_process(struct input_event*e){u32 nowsec=(u32)e->tv_sec,nowusec=(u32)e->tv_usec;if(e->type==EV_SYN&&e->code==SYN_DROPPED){qreset();suppress_tail=0;burst_hits=0;burst_guard=0;release_protect("STATE=SYN_DROPPED_FAIL_OPEN\\nPROTECT_ACTIVE=0\\nFAIL_OPEN=1\\n");process(e);return;}maybe_arm_on_frame(e);if(!protect_active){process(e);return;}if(suppress_tail){process(e);if(e->type==EV_SYN&&e->code==SYN_REPORT)suppress_tail=0;return;}if(!qactive&&contact_start_event(e)){burst_decay(nowsec,nowusec);qactive=1;qsec=nowsec;qusec=nowusec;qcount=0;qpassthrough=0;qmulti=0;}if(!qactive){process(e);forward_event(e);return;}if(qpassthrough){process(e);forward_event(e);if(last_end_happened){u32 elapsed=timeval_diff_us(qsec,qusec,last_end_sec,last_end_usec);burst_note(last_end_sec,last_end_usec,elapsed,last_end_risk);capture_contact(last_end_sec,last_end_usec,elapsed,last_end_risk,last_end_families,qcount,qmulti,"ALLOW_PASSTHROUGH");qreset();}return;}if(qcount>=QMAX){qflush();qpassthrough=1;process(e);forward_event(e);return;}qbuf[qcount++]=*e;process(e);if(raw_mt_count>1)qmulti=1;{u32 elapsed=timeval_diff_us(qsec,qusec,nowsec,nowusec);u32 hold=protect_hold_us();if(qmulti){qflush();qpassthrough=1;return;}if(last_end_happened){int classic_drop=(last_end_candidate&&elapsed<=hold);burst_note(last_end_sec,last_end_usec,elapsed,last_end_risk);if(classic_drop){capture_contact(last_end_sec,last_end_usec,elapsed,last_end_risk,last_end_families,qcount,qmulti,"BLOCK_CLASSIC");log_block(last_end_sec,last_end_usec,last_end_risk,last_end_families);qreset();suppress_tail=1;return;}if(burst_should_drop(elapsed,last_end_risk,last_end_families)){capture_contact(last_end_sec,last_end_usec,elapsed,last_end_risk,last_end_families,qcount,qmulti,"BLOCK_BURST");log_block_burst(last_end_sec,last_end_usec,last_end_risk,last_end_families);qreset();suppress_tail=1;return;}capture_contact(last_end_sec,last_end_usec,elapsed,last_end_risk,last_end_families,qcount,qmulti,"ALLOW");qflush();qreset();return;}if(elapsed>=hold){qflush();qpassthrough=1;return;}}}'''

for old, new, label in (
    (old_release, new_release, "release"),
    (old_log, new_log, "block log"),
    (old_q, new_q, "adaptive queue + capture"),
    (old_process, new_process, "adaptive protect_process + capture"),
):
    if old not in src:
